%{
--------------------------------------------------------------------------
FUNCTION NAME: figures_spike_pct_change_violin

DESCRIPTION:
    Vertical violin plot of channel-wise spike rate percent change (baseline
    vs DOI). Loads rates from preprocess cache (same as figures_spike_rate.m).
    Drops channels with 0 spikes/min on both baseline and treatment. Includes baseline 0
    with treatment greater than 0 (undefined ratio capped to pctMaxInclude when set, else 1e6).
    Optional excludeSilencedPct: if true, drops pct ~ -100%%
    (silenced spike rate); if false (default), keeps them in the violin.
...
OUTPUTS:
    Figure saved to pipeline/figures/spike_pct_change_violin_doi.png
--------------------------------------------------------------------------
%}
function figures_spike_pct_change_violin()
...
% If true, drop observations with pct ~ -100%% (treatment spike rate 0); if false, include them
...
for pairIdx = 1:nPairs
    ...
    % Exclude only both silent (0 spikes/min baseline and treatment); keep baseline 0 with treatment > 0
    keep = ~(b == 0 & t == 0);
    b = b(keep);
    t = t(keep);

    pctPair = 100 * (t - b) ./ b;
    % b==0, t>0 gives Inf; cap for ksdensity/boxplot and pctMaxInclude filter
    infMask = isinf(pctPair);
    if any(infMask)
        if ~isempty(pctMaxInclude)
            pctPair(infMask) = pctMaxInclude;
        else
            pctPair(infMask) = 1e6;
        end
    end
...
if isempty(pctAll)
    error('No valid channel observations (aligned channels; need at least one non-both-zero spike pair per pool).');
end
...
if excludeSilencedPct
    ...
    fprintf('Excluded silenced (-100%%): %d  |  Remaining: %d\n', nSilenced, numel(pctData));
    if isempty(pctData)
        error('No observations after excluding -100%% silenced channels.');
    end
else
    pctData = pctAll;
    fprintf('Spike %% change observations (silenced -100%% included): %d\n', numel(pctData));
end
...
ylabel('Percent change in spike rate (%)', 'FontSize', 18, 'FontName', 'Arial', 'FontWeight', 'bold');
...
filename = 'spike_pct_change_violin_doi.png';
