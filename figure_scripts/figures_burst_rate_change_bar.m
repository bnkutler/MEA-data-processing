%{
--------------------------------------------------------------------------
FUNCTION NAME: figures_burst_rate_change_bar

DESCRIPTION:
    Boxplots of per-dataset percentages (silenced / decreased / increased) for
    baseline-active channels (burst rate from cache, same pipeline as
    figures_burst_rate.m). Uses the same ignoreSilentChannels / minRateThreshold
    (bursts/min, cache burstRates) channel filter when those options are enabled.
    For each aligned
    channel with baseline burst rate > 0:
        Silenced  — treatment burst rate == 0 (was active, now silent)
        Decreased — treatment rate > 0 and < baseline (still active, lower)
        Increased — treatment rate > baseline
    Channels with baseline burst rate == 0 are excluded.
    Unchanged (within tolerance) is excluded from the percentage denominator
    (n = silenced+decreased+increased per pair); unchanged counts remain in console.
...
OUTPUTS:
    Figure saved to pipeline/figures/burst_rate_change_categories.png
...
--------------------------------------------------------------------------
%}
function figures_burst_rate_change_bar()
...
% Colors (match figures_spike_rate_change_bar.m)
...
minRateThreshold = 0.0;  % bursts/min; if 0 with ignoreSilentChannels, only drop 0 & 0 burst pairs
...
for pairIdx = 1:length(baselineList)
    ...
    % Only consider channels that were active at baseline.
    use = b > 0;
    b2 = b(use);
    t2 = t(use);
    if isempty(b2)
...
if nClassified == 0
    error('No valid channel observations (need baseline burst rate > 0 and aligned channels in cache).');
end
...
%   Title:   Distribution of channel response categories across recordings (burst rate)
%--------------------------------------------------------------------------
...
filename = 'burst_rate_change_categories.png';
