%{
--------------------------------------------------------------------------
FUNCTION NAME: figures_spike_rate

DESCRIPTION:
    Generates two figures for spike rate (spikes/min) per channel (baseline
    ...
    markers and vertical IQR (Q1-Q3) bars at each condition, and
    ...
    as figures_spike_percent_change.m and process_data.m.
...
OUTPUTS:
    ...
    Optional: spike_rate_data.mat with rates and channel list
--------------------------------------------------------------------------
%}
function figures_spike_rate()
...
% Processing parameters (matching figures_spike_percent_change.m and process_data.m)
STORE = 'Wav1';
BP_FILTER = [300 2500];
max_harmonic = 780;
notch_base = 60;
...
%----------------------------------------------------------------------
CHANNELS = 1:64;
%CHANNELS = 1:20;  % example: subset of channels
ignoreSilentChannels = true; % if true, drop channels with 0 baseline AND 0 DOI rates
minRateThreshold = 1.0;      % spikes/min; drop channels where both baseline and DOI are below this value
excludeMeanMultiplierOutliers = true; % if true, drop paired rows where either rate > multiplier * mean for that condition
...
%==============================================================================
% Figure 1: Paired-line (spike rate)
%==============================================================================
...
if max(fT) > 0
    ...
    yT = yGrid(:);
    % Right edge: (1.24 + fT, yT) must pair by index; use flipud(fT) so patch and outline match y
...
statsLabels = sprintf('\\bfActive channels:\\rm\n\\bfChannels with increased firing:\\rm\n\\bfChannels with decreased firing:\\rm');
...
filename1 = 'spike_rate_baseline_vs_treatment.png';
...
filename2 = 'spike_rate_boxplot.png';
...
%--------------------------------------------------------------------------
% Get spike rates (spikes/min) per channel for a dataset (filter + TDTthresh)
% Returns [spikeRates, durationSec]. Same pipeline as getSpikeCountsPerChannel.
%--------------------------------------------------------------------------
function [spikeRates, durationSec] = getSpikeRatesPerChannel(BLOCKPATH, STORE, CHANNELS, BP_FILTER, max_harmonic, notch_base, Q)
    nCh = length(CHANNELS);
    ...
    % Convert counts to spikes/min (use first valid duration; if all failed, durationSec stays NaN)
    if isnan(durationSec) || durationSec <= 0
        spikeRates = NaN(nCh, 1);
    else
        spikeRates = spikeCounts * (60 / durationSec);
    end
end
